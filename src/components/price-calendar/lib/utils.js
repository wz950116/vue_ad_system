...
var getData = function (price, year, month) {
    ...
    var ly = parseInt(year) || new Date().getFullYear();
    var lm = parseInt(month) || new Date().getMonth() + 1;
...
var getMonthData = function (y, m, priceData) {
    ...
    var d1 = new Date(y, m - 1); //要显示的日期
    //alert("要显示的日期"+d1);
    var i1 = d1.getDay(); //这个月的第一天是星期几
    var len = 5;
    ...
    for (var j = 1; j <= len; j++) {
        var week = [];
        for (var k = 1; k <= 7; k++) {
            var i2 = (j - 1) * 7 + k - i1;//alert("i2=" + i2);i2为格子的位置已当月第一天为1，前为0，-1，-2，后为2,3,4
            if (i2 < 1 || i2 > maxdays) {
                i2 = 0;
            }
            var obj = {date: i2}
            var i3 = Date.parse(new Date(y, m - 1, i2));//当前格子的日期
